...
const DEFAULT_PLUGIN_DATA_DIR = path.join(os.homedir(), ".claude", "plugins", "data", "ensemble-cursor");
const PREMIUM_PRESETS = new Set(["premium", "reasoning", "fast"]);

/** Print usage help for the companion entrypoint. */
export function printUsage() {
  process.stdout.write(
    "Usage: cursor-companion.mjs <task|status|result|cancel|review|adversarial-review|setup> [options]\n",
  );
}

/** Set the Cursor plugin data root expected by the shared state helpers. */
export function ensureCursorPluginDataEnv(env = process.env) {
...
/** Build the warning line for premium Cursor model presets. */
export function buildQuotaWarning(modelPreset) {
  if (!PREMIUM_PRESETS.has(modelPreset)) {
    return null;
  }
  return `QUOTA WARNING: preset '${modelPreset}' dispatches ${resolveModelPreset(modelPreset)}.`;
}
...
export function summarizePrompt(prompt, fallback = "Cursor task") {
...
/** Build a plain diff review prompt for Cursor Agent. */
...
/** Execute one Cursor CLI request and return the parsed output. */
export async function runCursorInvocation({ cwd, prompt, modelPreset = "auto", worktree = false, readOnly = false, noAgents = false }) {
  const argv = cursorCliAdapter.buildArgv({
  ...
  const child = cursorCliAdapter.spawn(argv, process.env, cwd);
  const outcome = await collectChildOutput(child);
  const parsed = cursorCliAdapter.parseOutput(outcome.stdout, outcome.stderr);

  if (outcome.exitCode !== 0 || !parsed.ok) {
    const reason = parsed.errorMessage ?? parsed.stderr ?? `Cursor Agent exited with code ${outcome.exitCode ?? "unknown"}.`;
...
export async function executeTaskJob(job, options = {}) {
  return runTrackedJob(job, async ({ reportProgress, updateProgress }) => {
    reportProgress({ message: "Dispatching Cursor task.", phase: "running" });
    const parsed = await runCursorInvocation({
      cwd: options.cwd ?? job.request.cwd ?? job.workspaceRoot,
      prompt: job.request.prompt,
      modelPreset: job.request.modelPreset ?? "auto",
      worktree: job.request.worktree ?? false,
      readOnly: false,
    ...
    const rendered = renderTaskResult(parsed, { title: "Cursor Task Result" });
    return {
      result: parsed,
      rendered,
    };
...
export function buildSetupReport(cwd) {
  ...
  const checks = [
    { label: "node", ok: true },
    { label: "cursor-agent", ok: fs.existsSync(CURSOR_AGENT_BIN) },
    { label: "cursor-api-key", ok: Boolean(process.env.CURSOR_API_KEY) },
    { label: "auth", ok: auth.ok },
  ];
  const nextSteps = [];
  if (!checks[1].ok) {
    nextSteps.push(`Install or restore ${CURSOR_AGENT_BIN}.`);
  }
  if (!checks[2].ok) {
    nextSteps.push("Export CURSOR_API_KEY before using the Cursor arm.");
  }
  if (checks[1].ok && checks[2].ok && !auth.ok) {
    nextSteps.push("Fix CURSOR_API_KEY or Cursor account access, then rerun /cursor:setup.");
  }
  if (auth.ok) {
    nextSteps.push("Use /cursor:rescue to dispatch work.");
  }
...
export async function handleTask(argv) {
  ...
  const modelPreset = parsed.model ?? "auto";
  resolveModelPreset(modelPreset);
  const quotaWarning = buildQuotaWarning(modelPreset);
  ...
  if (parsed.background) {
    ...
    process.stdout.write([quotaWarning, `started ${job.id}`, `pid ${pid}`, `status /cursor:status ${job.id}`].filter(Boolean).join("\n") + "\n");
    return;
  }

  const finished = await executeTaskJob({ ...job, pid: process.pid }, { cwd });
  const storedJob = loadStoredJob(job.workspaceRoot, job.id);
  process.stdout.write([quotaWarning, renderStoredJobResult(finished, storedJob)].filter(Boolean).join("\n"));
}
...
export async function handleReviewCommand(argv, mode) {
  ...
  const job = createJobRecord({
    kind: mode,
    kindLabel: mode === "adversarial-review" ? "Cursor adversarial review" : "Cursor review",
    title: mode === "adversarial-review" ? "Cursor adversarial review" : "Cursor review",
    workspaceRoot: resolveWorkspaceRoot(cwd),
    summary: target.label,
    request: { prompt, cwd, modelPreset: "auto", worktree: false },
    write: false,
    jobClass: "cursor-review",
  }, { prefix: "cursor" });
  ...
  const finished = await runTrackedJob(trackedJob, async ({ reportProgress, updateProgress }) => {
    ...
    const parsedOutput = await runCursorInvocation({ cwd, prompt, modelPreset: "auto", worktree: false, readOnly: true });
...
export async function main(argv = process.argv.slice(2)) {
  ensureCursorPluginDataEnv(process.env);
